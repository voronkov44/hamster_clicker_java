...
<main>
  ...
  <img id="clickableImage" src="<%= request.getContextPath() %>/photo/hamster_NY.png" alt="Хомяк" />
  <div id="counter">Счет: 0</div>
  <audio id="audio" src="<%= request.getContextPath() %>/audio/NY.mp3" preload="auto"></audio>
</main>
...
<script>
  ...
  function endGame() {
    ...
    window.location.href = `<%= request.getContextPath() %>/result.jsp?clicks=${clicks}`;
  }
